...
# Cross-CSV column-naming standardization. Applied just before writing so
# every output uses one name per concept regardless of which builder
# produced it. loans.csv carries the raw sectG column names (Branch Channel,
# Product Type, LoanAmount); aggregates are conformed to those names so
# Gallus can join everything by identical keys.
_GLOBAL_RENAMES = {
    "channel":     "Branch Channel",
    "product":     "Product Type",
    "total_loans": "loan_count",
    "avg_prob":    "avg_probability",
    "total_ev":    "total_expected_value",
}

_FILE_RENAMES = {
    "cycle_times_by_product.csv":   {"count": "loan_count"},
    # moneyball uses both: per-row `count` (becomes loan_count) and a global
    # grand-total `total_loans` that we keep distinct to avoid collision.
    "moneyball_quadrant_summary.csv": {
        "count": "loan_count",
        "total_loans": "total_loans_all_quadrants",
    },
}
...
def _write(df: pd.DataFrame, name: str) -> None:
    path = OUT_DIR / name
    rename = {**_GLOBAL_RENAMES, **_FILE_RENAMES.get(name, {})}
