"""
FlexPoint Loan Funding Forecasting — Feature Engineering

Builds snapshot-based training data for ML models.  Each training row
represents one active loan observed at one snapshot date, with features
knowable at prediction time and a binary target: did the loan fund by
that month's end?

The core function `build_feature_row()` is shared between training
(historical snapshots) and scoring (live pipeline) to eliminate
train/serve skew.
"""
import sys
...
def build_feature_row(active_df, as_of, month_end, transition_tables):
    ...
    ----------
    active_df : DataFrame
        Active loans; must already have current_stage, stage_rank,
        days_at_stage columns (from vectorized_current_stage).
    as_of : Timestamp
    month_end : Timestamp
    ...
    Returns
    -------
    DataFrame with NUMERIC_FEATURES + CATEGORICAL_FEATURES columns.
    """
    ...
    # v2: Use unstratified (stage-only) so product/purpose stay as independent
    # categorical features with their own predictive weight.
    ...
    # is_locked: HasRateLock==1 AND Rate Lock D <= as_of (guard future-dated)
    ...
    # days_until_lock_expiry: (expiry - as_of).days; NaN if not locked
    ...
    # lock_already_expired: expiry < as_of (Augie's "in trouble" signal)
    if rate_lock_exp is not None:
        lock_expired = (
    ...
    # days_since_lock: (as_of - Rate Lock D).days; NaN if not locked
    ...
    # lock_expiry_vs_month_end: (expiry - month_end).days; negative = expires
    # before month end. NaN if no expiry date.
    if rate_lock_exp is not None:
    ...
    features = pd.DataFrame({
        "stage_rank":               active_df["stage_rank"].values,
        "days_at_stage":            active_df["days_at_stage"].values,
...
def build_training_set(df, transition_tables, months=None):
    ...
    For each (month, day) in months × SNAPSHOT_DAYS, reconstruct
    the active pipeline among completed loans, compute features, and
    record whether each loan funded by month-end.

    Parameters
    ----------
    months : list of (year, month) tuples, optional.
        If None, uses config.BACKTEST_MONTHS.

    Returns DataFrame with features + metadata + target.
    """
    ...
    for year, month in month_list:
        for day in config.SNAPSHOT_DAYS:
            if day == 0:
                # Day-0: as_of is last day of prior month
            ...
            # ── Active loan identification (same pattern as transition_tables) ─
            opened = (
...
def encode_categoricals(df, encoders=None, fit=True):
    ...
    fit=True  : learn categories from data (training)
    fit=False : reuse stored categories (scoring), add 0-cols for unseen
    """
    df = df.copy()
    if encoders is None:
        encoders = {}
...
if __name__ == "__main__":
    ...
    print(f"{'═' * 60}")
    print(f"  Shape: {training.shape[0]:,} rows × {training.shape[1]} columns")
    print(f"  Positive rate (fund_by_end): {training['fund_by_end'].mean():.1%}")
    print(f"  Snapshots: {training.groupby(['snapshot_year','snapshot_month','snapshot_day']).ngroups}")
    print(f"\n  By year:")
    for yr in sorted(training["snapshot_year"].unique()):
        sub = training[training["snapshot_year"] == yr]
        print(f"    {yr}: {len(sub):,} rows  pos rate {sub['fund_by_end'].mean():.1%}")

    print(f"\n  Feature columns (before encoding): {len(NUMERIC_FEATURES)} numeric + "
          f"{len(CATEGORICAL_FEATURES)} categorical")

    # Encode and show final feature count
